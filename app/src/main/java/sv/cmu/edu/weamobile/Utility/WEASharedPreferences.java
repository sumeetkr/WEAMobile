...
//    }
    ...
    public static void updateAlertState(Context context, AlertState alertState) {
        SharedPreferences sharedPreferences = context.getSharedPreferences(Constants.SHARED_PREFERENCES, Activity.MODE_PRIVATE);
        if (sharedPreferences != null) {
            String res = sharedPreferences.getString(Constants.ALERT_STATE + alertState.getId(), null);
            if(res==null){
                saveAlertState(context, alertState);
            }
        }
    }
